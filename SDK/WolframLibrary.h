...
#define WolframLibraryVersion 8
...
#if !defined(MINT_32) && !defined(__LP64__) && !defined(_WIN64)
#define MINT_32
#endif
...
struct st_WolframLibraryData
{
	...
	mbool (*registerInputStreamMethod)(const char* name, void (*ctor)(MInputStream, const char* msgHead, void* optionsIn),
	...
	mbool (*registerOutputStreamMethod)(const char* name,
	...
	struct st_WolframTabularColumnLibrary_Functions* tabularColumnLibraryFunctions;
	...
	*/
	void* (*WL_malloc)(size_t size);

	/*
		Allocates a block of size bytes of aligned memory.
		The content of the newly allocated block of memory is not initialized, remaining with indeterminate values.
		If size is zero and in case of failure, a null pointer is returned.
	*/
	void* (*WL_malloc_aligned)(size_t size);

	/*
		Reallocates the given area of memory. It must be previously allocated by WL_malloc and not yet freed with
	...
	*/
	void* (*WL_realloc)(void* ptr, size_t new_size);

	/*
		Reallocates the given area of aligned memory. It must be previously allocated by WL_malloc_aligned and not yet
		...
		If new_size is zero, a null pointer is returned (in which case the old memory block may not be freed).
	*/
	void* (*WL_realloc_aligned)(void* ptr, size_t new_size);

	/*
		Deallocates a block of memory previously allocated by a call to WL_malloc, WL_malloc_aligned, WL_realloc or
